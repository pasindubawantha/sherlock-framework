...
#define CONTEX "LSTMCNNFC-Prediction"
#include "logger.hpp"
#include <math.h>   
...
int LSTMCNNFCPredictionModel::predict(int points, std::string expect, std::string predict, double lstmW, double cnnW, int abs) {
    ...
    logger::log(CONTEX, predict);
    logger::log(CONTEX, expect);
...
int LSTMCNNFCPredictionModel::predict(int points, std::string expect, std::string predict, int simVecSize, double marker, double simMargin, double lstmW, double cnnW, int abs) {
...
int LSTMCNNFCPredictionModel::predictNorm(int points, std::string expect, std::string predict, double lstmW, double cnnW) {
    ...
    for (int i = 0; i < inputSize; i++) {
        ...
        inVec = dataproc->process(inVec,0);
        // logger::log("### DEBUG ###", std::to_string(inVec[0]));
    ...
    for (int i = inputSize; i < predSize; i++) {
        ...
        // logger::log("### DEBUG CNN final ###", std::to_string(val));
        // logger::log("### DEBUG LSTM final ###", std::to_string(result));
...
int LSTMCNNFCPredictionModel::predictNorm(int points, std::string expect, std::string predict, int simVecSize, double marker, double simMargin, double lstmW, double cnnW) {
...
int LSTMCNNFCPredictionModel::predictAdaptNorm(int points, std::string expect, std::string predict, int timeLimit, int simVecSize, double marker, double simMargin, double lstmW, double cnnW) {
...
int LSTMCNNFCPredictionModel::predictFromFile(std::string infile, int points, std::string predict, double lstmW, double cnnW, int abs) {
...
int LSTMCNNFCPredictionModel::dtwSimilarity(int points, std::string expect, std::string predict, int simVecSize, double lstmW, double cnnW, int abs) {
    ...
    Eigen::MatrixXd prediction;
    // Open the file to write the time series predictions
    std::ofstream out_predict;
    out_predict.open(predict,std::ofstream::out | std::ofstream::trunc);
    std::ofstream out_expect;
    out_expect.open(expect,std::ofstream::out | std::ofstream::trunc);
    ...
    for (int i = inputSize; i < predSize; i++) {
        ...
        bool flag = false;
        double nonval;
        for(int t=0;t<expectedVec.size();t++){
            if(std::isnan(expectedVec[t])){
                flag = true;
                nonval = expectedVec[t];
            }
        }
        for(int t=0;t<predictedVec.size();t++){
            if(std::isnan(predictedVec[t])){
                flag = true;
                nonval = predictedVec[t];
            }
        }
        ...
        if(flag){
            ...
            similarity = DTW::getSimilarity(expectedVec,predictedVec);
            // similarity = DTW::eucledian(expectedVec,predictedVec);
        }
         
        out_expect<<similarity<<"\n";
        out_predict<<result<<"\n";
        // std::cout << ".";
    }
