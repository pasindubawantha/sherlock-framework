...
input_dir = "../data/nab_tuned_results_noCDD_training_ratio/"
model_summary_prefix = "../data/nab_tuned_results_noCDD_training_ratio/sherlock-noCDD_list"
...
for prediction_training_ratio_fraction in prediction_training_ratio_fractions:
    model_summary_file = model_summary_prefix + "_training_ratio_" + str(prediction_training_ratio_fraction) + ".csv"
    model_dataframe = pandas.read_csv(model_summary_file, index_col="file")

    iteratoin_input_dir = input_dir + "/training_ratio_"+str(prediction_training_ratio_fraction) + "/"

    reg_x = re.compile(r'\.(csv)')
    csv_input_files = []
    for path, dnames, fnames in os.walk(iteratoin_input_dir):
    ...
    for f in csv_input_files:
        ...
        fout = iteratoin_input_dir + '/' + f.split('/')[-1]
        print(fout)
